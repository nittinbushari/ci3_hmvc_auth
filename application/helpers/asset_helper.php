...
if (!function_exists('vendor')) {
	function vendor($type, $file)
	{
		...
		if ($type == 'css') {
			echo '<link rel="stylesheet" href="' . base_url() . 'themes/' . $theme . '/js/vendor/' . $file . '">';
		} elseif ($type == 'js') {
			echo '<script src="' . base_url() . 'themes/' . $theme . '/js/vendor/' . $file . '" ></script>';
		}
	}
}
...
if (!function_exists('css')) {
	function css($file, $module="")
	{
		if($module){
			$ci = &get_instance();
			$theme = $ci->template->get_theme();
			$theme_path = base_url().'themes/'.$theme.'/modules/'.$module;
			return '<link rel="stylesheet" href="'.$theme_path.'/css/'.$file.'">';
		}else{
			return asset_css($file);
		}
...
if (!function_exists('js')) {
	function js($file,$module="")
	{
		if($module){
			$ci = &get_instance();
			$theme = $ci->template->get_theme();
			$theme_path = base_url().'themes/'.$theme.'/modules/'.$module;
			return '<script src="'.$theme_path.'/js/'.$file.'"></script>';
		}else{
			return asset_js($file);
		}
...
if (!function_exists('img')) {
	function img($file,$return =false , $args=array())
    {
		...
		if($return){
           ...
           return '<img src="' . base_url() . 'themes/' . $theme . '/img/' . $file . '?ver=1.0" '.$tags.' >';
		}
...
if (!function_exists('asset_image')) {
	function asset_image($file = "", $https = false, $return = false, $args = array())
	{
		$CI = &get_instance();
		$CI->load->library('asset');
		return $CI->asset->asset_html_image($file, $https, $return, $args);
	}
}
