...
class Auth extends Front_Controller {
	...
	public function userWidget(){
		echo $this->load->view('auth/user-widget',true);
	}
	...
	public function create_user()
    {
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            $additional_data = array(
                ...
                'created_datetime'		=> $this->input->post('alias')
            );
        }
        if ($this->form_validation->run() == true)
        {
			$is_valid = $this->my_recaptcha->is_valid();
        ...
        else
        {
			...
			$this->data['alias'] = array(
                ...
                'class' =>  'form-control',
                'placeholder'=>'Alias',
                'value' => $this->form_validation->set_value('alias'),
            );
            $this->data['identity'] = array(
                'name'  => 'identity',
                'id'    => 'identity',
    ...
    public function register()
    {
        ...
        if ($this->form_validation->run() == true)
        {
			...
			$is_valid = $this->my_recaptcha->valid();
			...
			if($is_valid['success']){
				...
				message('message',"reCAPTCHA not solved/an error occured");
				redirect('register');
				exit;
			}
       ...
       if (isset($_POST) && !empty($_POST))
		{
			...
			if ($this->form_validation->run() === TRUE)
			{
				...
				// 	$upload_id = $this->pages_m->insert('gallery', array('title' => $filename, 'link' => $filename));
                ...
                //         redirect('profile');
                //     }else{
                //         $data['avatar']=base_url().'uploads/'.$upload['msg']['file_name'];
                //     }
				// }
